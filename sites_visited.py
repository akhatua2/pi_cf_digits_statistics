...
import argparse
from utils import getRandDistribution, getInputFiles, getMoves, getSingleRandomSitesVisited, getSinglePiSitesVisited
import random
...
def randWalk(mod: int) -> list:
    ...
    for _ in tqdm(range(args.num_runs)):
        ...
        avg_distances.append(getSingleRandomSitesVisited(rand_distribution, moves, mod))
...
def piWalk(pi_cf_digits: list, mod: int) -> list:
    ...
    for i in tqdm(range(args.num_runs)):
        avg_distances.append(getSinglePiSitesVisited(groups[i], moves, mod))
    return avg_distances
